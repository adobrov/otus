...
class SelectContext {
    ...
    override fun toString(): String {
        if (columns.isNotEmpty()) {
            return "select " + columns.joinToString(", ")
        }
        return "select *"
    }
}
...
fun select(col: MutableList<String>) = SelectContext().apply { columns = col }.toString()
fun from(table: String) = FromContext().apply { tableName = table }.toString()

fun query(init: FromContext.() -> Unit) = SqlSelectBuilder().apply {
    s = select()
}
